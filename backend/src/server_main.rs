...
async fn watch_one_tree(
    ...
    let mut watch = match repo.watch_status_from(baseline).await {
        ...
        Err(e) => {
            ...
            // A placement-time status read that fails is exactly the condition
            // `statusUnavailable` exists for, and it is the same frame the
            // already-live error path publishes (#49). We publish the PLACEMENT
            // error rather than re-reading status: a re-read that happens to
            // succeed would report a healthy repository while no watch exists.
...
/// clean — the caller reports what actually happened AND that the fanout did
/// not, which is the truth and is recoverable (the client refetches).
///
