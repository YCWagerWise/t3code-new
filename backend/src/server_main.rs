...
async fn ensure_thread_on_shell(state: &AppState, command: &Value) -> Result<(), String> {
        ...
        None => match state.rt.projects().await {
                ...
                .and_then(|v| v.as_str())
            {
                ...
                None => {
                    ...
                    return Err("no seed project in store; refusing to invent an id".into());
                }
            },
            Err(e) => {
    ...
    let existing_row = state
        ...
        .try_threads()
        .await
        .map_err(|e| format!("thread store unreadable: {e}"))?
        .into_iter()
...
async fn publish_config(state: &AppState, item: Value) -> Result<(), String> {
    ...
    state
        ...
        .config_publish(&item)
        .await
        .map_err(|e| format!("config publish failed: {e}"))
}
...
async fn handle_request(frame: &Value, tx: &mpsc::UnboundedSender<OutFrame>, state: &AppState) {
    ...
    match method {
        ...
        "server.upsertKeybinding" | "server.removeKeybinding" => {
            ...
            {
                tracing::error!(%e, "keybindings config publish failed");
                exit_failure(tx, &id, &e);
                return;
            }
        ...
        "server.updateSettings" => {
            ...
            .await
            {
                tracing::error!(%e, "settings config publish failed");
                exit_failure(tx, &id, &e);
                return;
            }
            if let Err(e) = publish_config(
            ...
            .await
            {
                tracing::error!(%e, "provider status config publish failed");
                exit_failure(tx, &id, &e);
                return;
            }
            exit_success(tx, &id, wire);
        ...
        "server.refreshProviders" => {
            ...
            .await
            {
                tracing::error!(%e, "provider refresh config publish failed");
                exit_failure(tx, &id, &e);
                return;
            }
        ...
        "server.updateProvider" => {
            ...
            .await
            {
                tracing::error!(%e, "provider update config publish failed");
                exit_failure(tx, &id, &e);
                return;
            }
            exit_success(tx, &id, json!({ "providers": providers }));
        ...
        "orchestration.dispatchCommand" => {
            ...
            if async_lane {
                if let Err(e) = ensure_thread_on_shell(&state, &command).await {
                    ...
                    exit_failure(tx, &id, &format!("thread.turn.start refused: {e}"));
                    return;
                }
...
async fn fail_turn_launch(state: &AppState, thread_id: &str, message: String) {
    tracing::error!(%thread_id, %message, "turn launch refused");
    ...
    let _ = emit_thread_event(
        ...
        json!({
            "threadId": thread_id,
            "activity": {
                "id": format!("turn-launch:{thread_id}:{created_at}"),
                "tone": "error",
...
fn run_turn(command: Value, model: ModelRef, state: AppState) {
    tokio::spawn(async move {
        ...
        // The durable thread record is the source for anything the command did
        // not repeat (mode, model selection, worktree). Use the SDK's typed,
        // fallible accessor — never the display/quarantine `threads()` helper —
        // because a corrupt row is not permission to run in the workspace root
        // with full-access defaults.
        let command_model_selection = command.get("modelSelection").cloned();
        let command_runtime_mode = command
            .get("runtimeMode")
            .and_then(Value::as_str)
            .map(str::trim)
            .filter(|s| !s.is_empty())
            .map(String::from);
        let command_interaction_mode = command
            .get("interactionMode")
            .and_then(Value::as_str)
            .map(str::trim)
            .filter(|s| !s.is_empty())
        ...
        let stored = match state.rt.thread_record(&thread_id).await {
            ...
            Ok(None) => {
                fail_turn_launch(
                    &state,
                    &thread_id,
                    format!("thread launch metadata unavailable: no durable thread row for `{thread_id}`"),
                )
                .await;
                return;
            }
            Err(e) => {
                fail_turn_launch(
                    &state,
                    &thread_id,
                    format!("thread launch metadata unreadable for `{thread_id}`: {e}"),
                )
                .await;
                return;
            }
        };
        let stored_model_selection = stored.model_selection.clone();
        ...
        // (#103). If neither the command nor the durable record names it, the
        // lifecycle metadata is incomplete and the turn is refused.
        let configured_instance = command_model_selection
            .as_ref()
        ...
        let Some(configured_instance) = configured_instance else {
            fail_turn_launch(
                &state,
                &thread_id,
                "thread launch metadata has no provider instance binding".to_string(),
            )
            .await;
            return;
        };
        ...
        // it (#73/#79).
        let runtime_mode =
            command_runtime_mode.unwrap_or_else(|| stored.runtime_mode.as_str().to_string());
        let interaction_mode =
            command_interaction_mode.unwrap_or_else(|| stored.interaction_mode.clone());
        ...
        let worktree = stored.worktree_path.clone();
        ...
        let options = {
            ...
            command_model_selection
                .as_ref()
                .and_then(|v| v.get("options"))
                .or_else(|| stored_model_selection.get("options"))
                .filter(|v| !v.is_null())
