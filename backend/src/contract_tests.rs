...
/// #186: settings/provider mutations acknowledge success only after the
/// durable config topic publish lands. Otherwise the editor socket sees
/// Success while every passive `subscribeServerConfig` surface stays stale.
#[tokio::test]
async fn update_provider_fails_when_config_publish_fails() {
    let (state, dir) = test_state().await;
    let pool = do_storage::DbPool::new(dir.join("data").join("threadruntime"));
    let db = pool.object_db("threadruntime", "main").await.unwrap();
    ...
    assert_eq!(
        exit["exit"]["_tag"], "Failure",
        "config publish failure must fail the mutation, not ack success: {exit}"
    );
    let defect = exit["exit"]["cause"][0]["defect"].as_str().unwrap_or("");
    assert!(
        defect.contains("config publish failed") || defect.contains("retained"),
        "failure names the config publish path: {exit}"
    );
}
...
/// #337: a first turn carries lifecycle metadata below
/// `bootstrap.createThread`; validating launch metadata before
/// `ensure_thread_on_shell` must read that same source instead of rejecting a
/// valid draft before the SDK can persist it.
#[tokio::test]
async fn first_turn_launch_reads_runtime_metadata_from_bootstrap_create_thread() {
    let (state, dir) = test_state().await;
    let wt = dir.join("wt-launch");
    ...
    request(
        ...
        json!({"input": {
            "type": "thread.turn.start",
            "threadId": "t-bootstrap-launch",
            "modelSelection": {"instanceId": "claudeAgent"},
            "message": {"text": "hi", "messageId": "m-bootstrap"},
            "bootstrap": {"createThread": {
                "projectId": "p-workspace",
                "title": "bootstrap launch",
    ...
    assert_eq!(
        exit["exit"]["_tag"], "Success",
        "bootstrap lifecycle metadata is valid launch authority: {exit}"
    );
    let stored = state.rt.threads().await;
    let thread = stored
        .iter()
        .find(|t| t["id"] == "t-bootstrap-launch")
        .expect("thread persisted from bootstrap");
...
async fn project_content_search_honours_every_option() {
    ...
    let value = &f[0]["exit"]["value"];
    assert!(
        value["skippedBinaryCount"].as_u64().unwrap_or(0) >= 1,
        "binary files are reported as policy skips, not silent search misses: {value}"
    );
    assert_eq!(
        value["skippedLargeCount"], 1,
        "oversized files are reported as policy skips, not silent search misses: {value}"
    );
    assert_eq!(
        value["unsearchedCount"], 0,
        "binary/large skips are not confused with unreadable files: {value}"
    );
...
#[tokio::test]
async fn subscribe_thread_refuses_unreadable_checkpoint_stack_instead_of_empty_checkpoints() {
    let (state, dir) = test_state().await;
    let broken = dir.join("broken-checkpoints");
    std::fs::create_dir_all(&broken).unwrap();
    std::fs::write(broken.join(".git"), "not a gitdir\n").unwrap();

    let mut row = thread_row_ck("t-broken-checkpoints");
    row["worktreePath"] = json!(broken.to_string_lossy().into_owned());
    state.rt.save_thread(&row).await.unwrap();
    ...
    assert!(
        ...
        "an unreadable checkpoint substrate must not be projected as a snapshot with checkpoints=[]: {frames:#?}"
    );
    let exit = frames
        .iter()
        .find(|f| f["_tag"] == "Exit")
        .expect("subscribeThread exits visibly");
    assert_eq!(
        exit["exit"]["_tag"], "Failure",
        "checkpoint substrate failure is a contract failure: {exit:#?}"
    );
    let body = serde_json::to_string(exit).unwrap();
    assert!(
        body.contains("checkpoint summaries unavailable")
            || body.contains("checkpoint substrate unavailable"),
        "the failure names checkpoint unavailability: {body}"
    );
