...
async fn drop_thread_session_table(dir: &std::path::Path) {
    let pool = do_storage::DbPool::new(dir.join("data").join("threadruntime"));
    let db = pool.object_db("threadruntime", "main").await.unwrap();
    db.execute("DROP TABLE thread_session", vec![]).await;
}
...
async fn stop_command_fails_when_sdk_cancel_fails() {
    ...
    db.execute("DROP TABLE agent_control", vec![]).await;
...
/// #186: a settings/provider RPC cannot report success when the durable config
/// topic publish failed. Passive clients rely on `subscribeServerConfig`; the
/// command's own return value is not a substitute for the projection.
#[tokio::test]
async fn provider_update_fails_when_config_publish_fails() {
    let (state, dir) = test_state().await;
    state
        .rt
        .config_publish(&json!({"kind": "seed"}))
        .await
        .expect("create config pubsub tables");
    let pool = do_storage::DbPool::new(dir.join("data").join("threadruntime"));
    let db = pool.object_db("threadruntime", "main").await.unwrap();
    ...
    let exit = drain(&mut rx)
        .into_iter()
        .find(|f| f["_tag"] == "Exit")
        .expect("updateProvider exits");
    assert_eq!(
        exit["exit"]["_tag"], "Failure",
        "config publish failure must not be acknowledged as provider update success: {exit}"
    );
    let defect = exit["exit"]["cause"][0]["defect"].as_str().unwrap_or("");
    assert!(
        defect.contains("config publish"),
        "failure names the config publish seam: {exit}"
    );
}

/// #237: a follow-up turn must not recover launch authority from product-local
/// defaults when the durable thread row is malformed. The SDK typed record is
/// the source of runtime mode, interaction mode, provider binding and worktree.
#[tokio::test]
async fn turn_launch_refuses_malformed_durable_thread_metadata() {
    let (state, _d) = test_state().await;
    state
        .rt
        .save_thread(&json!({
    ...
    request(
        ...
        json!({"input": {
            "type": "thread.turn.start",
            "threadId": "t-corrupt",
            "message": {"text": "hi", "messageId": "m-corrupt"},
        }}),
    )
    .await;
    let exit = drain(&mut rx)
    ...
    assert_eq!(
        exit["exit"]["_tag"], "Success",
        "the async command may acknowledge only after durable bootstrap admission: {exit}"
    );

    let mut events = Vec::new();
    let mut launch_failed = false;
    for _ in 0..20 {
        events = state.rt.events_after("t-corrupt", 0, 20).await.unwrap();
        launch_failed = events.iter().any(|e| {
            e.pointer("/event/type").and_then(Value::as_str) == Some("thread.activity-appended")
                && e.pointer("/event/payload/activity/kind")
                    .and_then(Value::as_str)
                    == Some("turn.launch-failed")
                && e.pointer("/event/payload/activity/payload/error")
                    .and_then(Value::as_str)
                    .map(|s| s.contains("runtimeMode"))
                    .unwrap_or(false)
        });
        if launch_failed {
            break;
    ...
    assert!(
        launch_failed,
        "malformed durable thread metadata must produce a lifecycle failure event, not a default-root/full-access run: {events:?}"
    );
}
...
async fn the_shell_snapshot_reads_projects_from_the_durable_store() {
    ...
    let runner_b = tools::open_workspace_shell(&tmp, data.clone())
        .await
        .expect("open second workspace shell");
    ...
    let rt_b = ThreadRuntime::open(shell_b, data.to_str().unwrap(), "main")
        .await
        .expect("open second thread runtime");
...
async fn turn_count_one_is_the_most_recent_turn_not_the_oldest() {
    ...
    // two turns of work.
    state.rt.save_thread(&thread_row_ck("t-order")).await;
...
async fn a_revert_never_touches_the_runtimes_own_state() {
    ...
    state.rt.save_thread(&thread_row_ck("t-scope")).await;
    {
        use agent_sdk_shell::Projector as _;
        turn_projector(&state, state.cwd.clone())
            .project(Lifecycle::TurnStarted {
...
async fn root_checkpointed_thread(
    ...
    state.rt.save_thread(&thread_row_ck(thread_id)).await;
    checkpoint_turn_start(state, &state.cwd, "turn-1").await;
...
async fn checkpoint_revert_reports_failed_event_write_failure() {
    let (state, dir) = test_state().await;
    init_git_repo(&state.cwd);
    state.rt.save_thread(&thread_row_ck("t-failed-event")).await;
    drop_thread_event_table(&dir).await;
...
async fn a_revert_discards_the_reverted_turns_transcript_and_frees_their_ordinals() {
    ...
    state.rt.save_thread(&checkpoint_thread("t-rev")).await;
