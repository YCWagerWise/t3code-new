...
async fn stop_command_fails_when_sdk_interrupt_state_is_unreadable() {
    ...
    // Keep a foreground command running in Hearth. If durable SDK cancellation
    // fails, the stop path must NOT Ctrl-C this process as a side effect.
    let runner = state.terminal.clone();
    let mut running =
        tokio::spawn(async move { runner.run("sleep 30", false, Some(25), false).await });
    tokio::time::sleep(std::time::Duration::from_millis(400)).await;
    ...
    assert!(
        tokio::time::timeout(std::time::Duration::from_millis(500), &mut running)
            .await
            .is_err(),
        "durable SDK cancel failed, so Hearth must not have been interrupted first"
    );
