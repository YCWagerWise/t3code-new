...
async fn filesystem_browse_and_open_in_editor_are_implemented() {
    ...
    // an absolute host path is not browse authority. This used to walk the
    // host filesystem and only failed here because the fixture path did not
    // exist; now it is refused before any parent directory is read.
    let (tx, mut rx) = mpsc::unbounded_channel();
    request(
        &state,
        &tx,
    ...
    assert!(
        msg.contains("path_escapes_workspace"),
        "host paths are refused before listing: {msg}"
    );
